...
public class ItemUseTracker {
    ...
    public void use(final ItemData usedItem, int itemId) {
        if (!canBeUse(usedItem, itemId)) {
            return;
        }
    ...
    private boolean canBeUse(final ItemData usedItem, int itemId) {
        // This way we can support custom item use duration too :D
        // Also wrap this since I don't trust myself enough.
        try {
