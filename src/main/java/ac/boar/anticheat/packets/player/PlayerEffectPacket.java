    ...
    public void onPacketSend(final CloudburstPacketEvent event, final boolean immediate) {
        ...
        if (event.getPacket() instanceof MobEffectPacket packet) {
            ...
            if (packet.getEvent() == MobEffectPacket.Event.ADD) {
                player.latencyUtil.addTransactionToQueue(player.lastSentId, () -> player.statusEffects.put(effect, new StatusEffect(effect, packet.getAmplifier(), packet.getDuration())));
            } else if (packet.getEvent() == MobEffectPacket.Event.REMOVE) {
                player.latencyUtil.addTransactionToQueue(player.lastSentId, () -> player.statusEffects.remove(effect));
            }
        }
    }
}