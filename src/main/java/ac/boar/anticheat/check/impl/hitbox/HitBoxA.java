...
public class HitBoxA extends PacketCheck {
    ...
    public void onPacketReceived(final CloudburstPacketEvent event) {
        ...
        final boolean tooFarOrInvalid = entity == null || entity.getServerPosition().distance(player.x, player.y, player.z) > 6.0 || entity.getPosition().distance(player.x, player.y, player.z) > 6.0;
        if (tooFarOrInvalid) {
            event.setCancelled(true);
            return;
        }
